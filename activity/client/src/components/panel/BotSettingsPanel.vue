...
<template>
  ...
  <RevealOnScroll tag="section" class="panel-section module-tabs-panel" :delay="35"><PanelTabNav v-model="activeTab" :tabs="tabs" :aria-label="$t('module.bot-settings')" /></RevealOnScroll>
  <RevealOnScroll tag="section" class="panel-section module-content-panel" :delay="60">
    ...
    <div v-else class="ai-beta-workspace">
      ...
      <section class="ai-beta-card"><header><div><span class="muxivo-coreation-kicker">{{ $t("settings.ai_beta_permissions") }}</span><h3>{{ $t("settings.ai_beta_permissions_title") }}</h3><p>{{ $t("settings.ai_beta_permissions_help") }}</p></div></header><div class="ai-beta-toggle-list"><label class="ai-beta-toggle"><span><strong>{{ $t("settings.ai_acknowledge") }}</strong><small>{{ $t("settings.ai_acknowledge_help") }}</small></span><button type="button" class="ai-beta-switch" :class="{ 'is-on': enforcement.beta_enforcement_acknowledged }" role="switch" :aria-checked="enforcement.beta_enforcement_acknowledged" @click="enforcement.beta_enforcement_acknowledged = !enforcement.beta_enforcement_acknowledged"><i /></button></label><label v-for="item in [['allow_automated_timeout', 'settings.ai_timeout'], ['allow_automated_kick', 'settings.ai_kick'], ['allow_automated_ban', 'settings.ai_ban']] as const" :key="item[0]" class="ai-beta-toggle" :class="{ disabled: !elevatedEnabled }"><span><strong>{{ $t(item[1]) }}</strong><small>{{ $t('settings.ai_requires_elevated') }}</small></span><button type="button" class="ai-beta-switch" :class="{ 'is-on': enforcement[item[0]] }" role="switch" :aria-checked="enforcement[item[0]]" :disabled="!elevatedEnabled" @click="enforcement[item[0]] = !enforcement[item[0]]"><i /></button></label></div></section>
      <section class="ai-beta-card"><header><div><span class="muxivo-coreation-kicker">{{ $t("settings.ocr_eyebrow") }}</span><h3>{{ $t("settings.ocr_heading") }}</h3><p>{{ $t("settings.ocr_help") }}</p></div></header><div class="ai-beta-toggle-list"><label class="ai-beta-toggle" :class="{ disabled: !ocrReady }"><span><strong>{{ $t("settings.ocr_toggle") }}</strong><small>{{ ocrReady ? $t("settings.ocr_toggle_help") : $t("settings.ocr_unavailable") }}</small></span><button type="button" class="ai-beta-switch" :class="{ 'is-on': enforcement.ocr_enabled }" role="switch" :aria-checked="enforcement.ocr_enabled" :disabled="!ocrReady" @click="enforcement.ocr_enabled = !enforcement.ocr_enabled"><i /></button></label></div><div v-if="enforcement.ocr_enabled" class="ai-beta-fields"><label><span>{{ $t("settings.ocr_failure") }}</span><select v-model="enforcement.ocr_failure_mode"><option value="SKIP">{{ $t("settings.ocr_failure_skip") }}</option><option value="REVIEW">{{ $t("settings.ocr_failure_review") }}</option></select></label><label><span>{{ $t("settings.ocr_frames") }}</span><input v-model.number="enforcement.ocr_max_gif_frames" type="number" min="1" max="24" /></label><label class="ai-beta-toggle"><span><strong>{{ $t("settings.ocr_empty") }}</strong></span><button type="button" class="ai-beta-switch" :class="{ 'is-on': enforcement.ocr_process_empty_result }" role="switch" :aria-checked="enforcement.ocr_process_empty_result" @click="enforcement.ocr_process_empty_result = !enforcement.ocr_process_empty_result"><i /></button></label></div></section>
