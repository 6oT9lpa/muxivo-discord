...
@router.get("/api/muxivo-core/settings")
async def get_muxivo_core_settings(guild_id: int = Query(gt=0), access_token: str = Depends(require_bearer_token)) -> dict[str, object]:
    return await service.get_settings(guild_id, access_token)


@router.get("/api/muxivo-core/media-policy")
...
@router.put("/api/muxivo-core/media-policy")
...
@router.delete("/api/muxivo-core/media-policy")
...
@router.get("/api/muxivo-core/metrics")
async def get_muxivo_core_metrics(guild_id: int = Query(gt=0), access_token: str = Depends(require_bearer_token)) -> dict[str, object]:
    return await service.get_metrics(guild_id, access_token)


@router.get("/api/muxivo-core/reviews")
async def get_muxivo_core_reviews(guild_id: int = Query(gt=0), status: str = Query("OPEN", pattern="^(OPEN|RESOLVED)$"), limit: int = Query(20, ge=1, le=50), offset: int = Query(0, ge=0), access_token: str = Depends(require_bearer_token)) -> dict[str, object]:
    return await service.list_review_items(guild_id, access_token, status, limit, offset)


@router.get("/api/muxivo-core/reviews/audit")
async def get_muxivo_core_review_audit(guild_id: int = Query(gt=0), limit: int = Query(20, ge=1, le=50), offset: int = Query(0, ge=0), access_token: str = Depends(require_bearer_token)) -> dict[str, object]:
    return await service.list_review_audit(guild_id, access_token, limit, offset)


@router.put("/api/muxivo-core/reviews/{item_id}")
async def update_muxivo_core_review(item_id: int, payload: AiModerationReviewUpdatePayload, access_token: str = Depends(require_bearer_token)) -> dict[str, object]:
    return await service.update_review_item(item_id, payload, access_token)


@router.post("/api/muxivo-core/simulate")
async def simulate_muxivo_core(payload: AiModerationSimulationPayload, access_token: str = Depends(require_bearer_token)) -> dict[str, object]:
    return await service.simulate(payload.guild_id, payload.message_text, access_token)


@router.put("/api/muxivo-core/channels")
async def save_muxivo_core_channels(payload: AiModerationChannelsPayload, access_token: str = Depends(require_bearer_token)) -> dict[str, object]:
    return await service.save_channels(payload, access_token)


@router.put("/api/muxivo-core/policy")
async def save_muxivo_core_policy(payload: AiModerationPolicyPayload, access_token: str = Depends(require_bearer_token)) -> dict[str, object]:
